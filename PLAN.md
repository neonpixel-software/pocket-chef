...
- [ ] **5.1 "Type it" entry point** — text field for pasting/typing a raw recipe, feeds Apple Intelligence to structure it into title/ingredients/steps, lands on the Phase 2 review screen pre-filled.
  ...
  Still open: equipment. The instruction to leave it out isn't reliable. It removed the loaf pan and skewer in one run, but they came back after two unrelated lines were added to the same sample. #82 adds an Equipment section so the model has somewhere to put it.
