...
- [ ] **5.1 "Type it" entry point** — text field for pasting/typing a raw recipe, feeds Apple Intelligence to structure it into title/ingredients/steps, lands on the Phase 2 review screen pre-filled.
  ...
  Amounts since #76: the model copies quantities as written and `IngredientAmountParser` resolves them, covering `1/2`, `1 1/2`, `½`, `1 ⅔` and `100g` (value 100, unit g). Capture uses greedy sampling, because default sampling sometimes garbled quantities ("1±³" for "1 1/2"), and the instructions ask the model to leave out equipment. Verified on 2026-09-25 against the real model on two samples: every fraction line parses, and loaf pans, skewers and skillets no longer appear as ingredients. Still open: descriptors used as units ("large", "yellow onion"; wording in the schema and instructions didn't change this over 4 runs).
