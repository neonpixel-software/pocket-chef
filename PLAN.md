...
- [ ] **4.1 Settings screen with storage toggle** — Local vs. iCloud choice, wired to SwiftData's CloudKit-backed store.
  ...
  Schema changes after the first CloudKit build ships need migration planning: CloudKit's production schema only allows adding fields, never renaming or removing them, and older app versions keep syncing records without the new fields. Until that build ships, attributes can still be added freely (e.g. `RecipeModel.equipment`, #82). Before shipping it, deploy the development schema to production with every attribute the first release needs, and plan later model changes around additive fields.
...
- [ ] **5.1 "Type it" entry point** — text field for pasting/typing a raw recipe, feeds Apple Intelligence to structure it into title/ingredients/steps, lands on the Phase 2 review screen pre-filled.
  ...
  On-device check (2026-09-25, macOS 26.6.2, M4 Pro, Apple Intelligence enabled): the flow works end-to-end in the app — a typed recipe lands correctly pre-filled on the review screen. Left unchecked because extraction quality doesn't yet meet the acceptance bar: on long, chatty recipes steps leak into the ingredient list, units and amounts get invented ("egg", "pcs", "none"; a pinch → 0.01), and "100g"-style amounts don't parse (issue #76). Check off once #76 is fixed and the sample set re-verified on device.
  Amounts since #76: the model copies quantities as written and `IngredientAmountParser` resolves them, covering `1/2`, `1 1/2`, `½`, `1 ⅔` and `100g` (value 100, unit g). Capture uses greedy sampling, because default sampling sometimes garbled quantities ("1±³" for "1 1/2"), and equipment goes in its own field (see #82 below). Verified on 2026-09-25 against the real model on two samples: every fraction line parses.
  Descriptors since #76: the model uses them as units ("1 large egg" → unit "large") and prompt wording didn't stop it, so `IngredientDescriptors` drops any unit that isn't a known measurement (English, Spanish, French, German, Dutch) and removes leading size words (large, medium, small, extra-large, jumbo) from the name. Verified on device: "1 large egg" → 1 egg, "2 medium potatoes" → 2 potatoes, "1 large yellow onion" → 1 yellow onion. Colour and variety words stay in the name, because "brown sugar" and "green onion" are different ingredients.
  Equipment since #82: telling the model to leave equipment out wasn't reliable (the loaf pan and skewer came back after two unrelated lines were added to the sample), so a recipe now has an Equipment list, and the schema gives the model an `equipment` field between ingredients and steps. The mapping drops equipment that isn't in the source text, and drops an ingredient without a measurement unit whose line or name matches an equipment item (ignoring case, accents and spacing), because the model sometimes lists a tool in both ("8x4-inch loaf pan" named "loaf pan"). The unit check keeps food the model also lists as equipment, such as "¼ cup butter" when a step says "butter the pan". An amount check doesn't work, because the model gives tool lines an amount of 1. It doesn't filter by a list of equipment words, which would drop real ingredients such as "wooden skewers for kebabs". Verified on device (2026-09-25): with greedy sampling, the zucchini sample's loaf pan and skewer, the banana bread's loaf pan and the pancakes' skillet and whisk all land in Equipment and not in Ingredients, and "8 wooden skewers for kebabs" stays an ingredient. With default sampling (3 runs each of the zucchini and banana samples) no equipment showed up as an ingredient. Putting `equipment` first or last in the schema made steps leak into the crêpe recipe's ingredients more often, so it sits in the middle.
  Still open: with greedy sampling the crêpe recipe's first step ("Mélanger la farine et le sel dans un saladier.") comes through as an ingredient. On main it didn't, but the pan ("poêle chaude légèrement huilée") did, and with default sampling main loops until it runs out of context in 3 of 5 runs, versus 0 of 5 on this branch, where 2 of 5 leak steps. The crêpe sample is unstable either way.
  Prose recipes since #76: the model often leaves the unit empty on prose lines ("1/3 cup melted butter"), so the mapping reads it from rawText after the amount, or from a word amount ("a pinch" → unit pinch, no amount; it used to become 0.01). Ingredients that appear nowhere in the source text are dropped, which removed an invented "eau" from a crêpe recipe. The converse risk: if the model rephrases both the line and the name (say, translates "250 g de farine" to "250g de flour" / "flour"), a real ingredient is dropped, and the review screen gives no sign of it. The "exactly as written" guide makes this unlikely, and none of the on-device samples hit it. Verified on device (2026-09-25) with a chatty banana bread, a French crêpe recipe and a tomato soup. Steps leaking into ingredients (issue item 1) did not reproduce on these samples; the original banana-bread text wasn't kept in the issue, so it is unconfirmed rather than fixed.
  Not supported: amounts with a thousands separator ("1,000 g"). A comma followed by one or two digits is read as a decimal comma ("2,5" → 2.5), anything else with a comma gives no amount.
  Not supported: the crêpe recipe's ingredient sentence ("Pour 15 crêpes il vous faut 250 g de farine…") came through as its first step. It isn't filtered, because real steps also list ingredients ("Add a 28 oz can of crushed tomatoes, 2 cups vegetable stock…"); the user removes it on the review screen.
  Not supported: compound amounts such as "1 cup plus 2 tablespoons". The on-device model can't combine them and returns a single wrong number (1 2/3 cups instead of 1 1/8). Capture doesn't try to correct this; the user fixes the amount on the review screen before saving. The review screen shows only amount, unit and name, not the source line, so the user has to compare against the original recipe.
...
- [ ] **6.1 "Paste a link" entry point** — fetch page content, hand to Apple Intelligence to extract just the recipe, land on the same review screen pre-filled.
  ...
  On-device check (2026-09-25, same Mac as 5.1): left unchecked — 3 of 4 real recipe sites tried (Allrecipes, Simply Recipes, Serious Eats) fail with `exceededContextWindowSize`: the 12,000-character plain-text cap is larger than the model's 4,096-token context, and most of that text is site navigation rather than the recipe (issue #74). BBC Good Food extracted correctly. HTTP error pages are also not rejected — a 404 page was captured as a recipe titled "ERROR 404" (issue #75). Check off once #74 and #75 are fixed and re-verified on device.
  Fetch-layer limits (#49) verified on device: a >2 MB page (the WHATWG HTML spec) shows the "too large" message; a windows-1252 page (served locally) decodes with accents intact; a plain-HTTP-only site (`http://neverssl.com`) shows the "secure connection" message in the running app.
  HTTP status since #75: non-2xx responses are rejected before the body is read and show the "Couldn't load that page" message. Verified on 2026-09-25 against the real fetcher: the budgetbytes.com 404 page and an HTTP 500 now throw `httpStatus`; BBC Good Food still fetches.
  Model input since #74: pages embedding a schema.org `Recipe` JSON-LD block feed that to the model instead, reformatted as title/ingredients/steps; otherwise the page's plain text, with scripts, navigation and other site chrome stripped, is the fallback. Either input is truncated to 6,000 characters before the model call so it fits the on-device context window.
...
- [ ] **7.1 String Catalog + automatic UI string resolution** — `Localizable.xcstrings` covering every user-facing UI string (screen chrome, alerts and error messages, and the preset tag names), translated to Spanish, French, German, and Dutch alongside the English base.
...
- [x] **7.3 View-model error messages localized** — the 5 capture-failure messages in `RecipeCaptureViewModel`/`RecipeURLCaptureViewModel` resolve via the same String Catalog.
  Acceptance: triggering a capture failure while the device language is set to a translated locale shows the translated message, not English. Catalog entries are unit-tested directly (`LocalizedCaptureErrorMessagesTests`), and verified on device (2026-09-25) with the app running in German: the typed-capture failure and all four URL-capture messages (too large, insecure connection, couldn't load, no recipe found) showed translated.
...
- [ ] **9.1 Seed data import** — one-off script loading existing public ingredient-density data into the database.
  ...
  Source: [USDA FoodData Central](https://fdc.nal.usda.gov/), SR Legacy dataset — its household portions give the gram weight of a known volume (one US cup, 236.5882365 ml), which the seed script divides down to grams per millilitre (PR #70). The unit lives in `food_portion.modifier`, not in `measure_unit_id` (always 9999 "undetermined" for SR Legacy) or `portion_description` (always empty): 1,691 of the 14,449 SR Legacy portion rows are exactly `amount=1, modifier="cup"` — e.g. "Wheat flour, white, all-purpose, enriched, bleached" 125 g, "Sugars, granulated" 200 g, "Butter, salted" 227 g, "Honey" 339 g, "Cocoa, dry powder, unsweetened" 86 g (checked against FDC's 2018-04 SR Legacy CSV). Flour is filed under "Wheat flour, …", not "Flour, …". Many staples carry qualified cup rows instead (`cup packed`/`cup unpacked` for brown sugar, `cup sifted`/`cup unsifted` for powdered sugar, `cup, crumbs` under white bread), so the script picks each ingredient's row explicitly rather than matching "cup" as a substring; ingredients with no cup row fall back to `tbsp`/`tsp` rows or are left for manual curation. FDC data is public domain, published under [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/), so it can ship in this public MIT-licensed repo. Do not seed from cooking sites' weight charts (e.g. King Arthur) or the FAO/INFOODS density database: the former are all-rights-reserved, the latter restricts commercial reuse (issue #50).
  Provenance: each seed row in the script carries its FDC ID and the portion it came from (e.g. `// FDC <id>, 1 cup = 125 g`), so any value can be re-checked against the source. No `sourceUrl` column — nothing reads it yet; add one only if curation needs it later.
